...
public update(): void {
  this.turistickaAgencijaService.updateTuristickaAgencija(this.dataTuristickaAgencija).subscribe(() => {
    this.snackBar.open('Uspesno izmenjena turisticka agencija: ' + this.dataTuristickaAgencija.naziv, 'OK', {
      duration: 2500
    })
  }),
    (error: Error) => {
      console.log(error.name + ' ' + error.message)
      this.snackBar.open('Doslo je do greske prilikom izmene turisticke agencije. ', 'Zatvori', {
        duration: 2500
      })
    };
